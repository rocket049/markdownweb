<html>
<head>
  <meta http-equiv="content-type" content="text/html;charset=utf-8"/>
  <meta name="viewport" content="width=device-width,initial-scale=1.0">
  <link type="text/css" rel="stylesheet" href="/style.css"/>
  <title>{{.title}}</title>
<head>
<body>
<div class="all" id='all'>
  <div class="screen" id="screen">
    <ul id="ul">
      {{range .ad}}<li class="screen"><img class="screen" src='{{.Img}}' alt='{{.Text}}' width='760' height='200' onclick='window.open("{{.Href}}")' /></li>{{end}}
    </ul>
    <ol></ol>
    <div id="arr">
      <span id="left">&lt;</span>
      <span id="right">&gt;</span>
    </div>
  </div>
</div>
<div><a href="/">首页</a></div>
<div style='text-align:left;'>
{{.body}}
</div>
<div style="background:#E8E7E3;padding:20px 0;">
  <div style="font-size:x-small;text-align:center">
	备案号：<a target="_blank" href="http://beian.miit.gov.cn">苏ICP备19034936号</a>/
    <a target="_blank" href="http://beian.miit.gov.cn">苏ICP备19034936号-1</a>
    &nbsp;&nbsp; 站长邮箱：fuhuizn@163.com&nbsp;&nbsp; 版权所有
  </div>
  <div style="width:300px;margin:0 auto;font-size:x-small;">
	<a target="_blank" href="http://www.beian.gov.cn/portal/registerSystemInfo?recordcode=32050702010684" style="display:inline-block;text-decoration:none;height:20px;line-height:20px;"><img src="/bei-an.png" style="float:left;"/><p style="float:left;height:20px;line-height:20px;margin: 0px 0px 0px 5px; color:black;">苏公网安备 32050702010684号</p></a>
  </div>
  <div style="font-size:x-small;text-align:center">Powered by MarkdownWeb</div>
</div>
<script src="/ad.js"></script>
</body>
</html>
